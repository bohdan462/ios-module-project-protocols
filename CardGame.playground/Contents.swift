...
enum RanckPlayingCards: Int { //Ranck
    
    case ace = 1
    case two, three, four, five, six, seven, eight, nine, ten, jack, queen, king
    
    static var allRanckes: [RanckPlayingCards] {
...
extension RanckPlayingCards: CustomStringConvertible { //Ext to RANCK
...
//let q: RanckPlayingCards = .eight
//print(q)
//: ## Step 3
//: Create an enum for the suit of a playing card. The values are `hearts`, `diamonds`, `spades`, and `clubs`. Use a raw type of `String` for this enum (this will allow us to get a string version of the enum cases for free, no use of `CustomStringConvertible` required).
enum SuitPlayingCards: String { //Suit
    case hearts, diamonds, spades, clubs
    
    static var allSuited: [SuitPlayingCards] {
...
struct Card {
    var suit: SuitPlayingCards
    var rank: RanckPlayingCards
}
...
//let w: Card = Card(suit: .clubs, rank: .eight)
//print(w)
...
struct Deck {
    ...
    init() {
        var array = [Card]()
        for suitValue in SuitPlayingCards.allSuited {
            for ranckValue in RanckPlayingCards.allRanckes {
                array.append(Card(suit: suitValue, rank: ranckValue))
            }
        }
        self.arrayOfCards = array
    }
    
    func drawCard() -> Card {
        let randomCard = arrayOfCards.randomElement()!
        return randomCard
    }
...
//var i = RanckPlayingCards.allRanckes
...
//var cards = Deck()
//print(cards.arrayOfCards.count)
//print(cards.drawCard())
//print()
//: ## Step 11
...
extension RanckPlayingCards: Comparable {
    static func < (lhs: RanckPlayingCards, rhs: RanckPlayingCards) -> Bool {
...
extension HighLow {
    ...
    func play() {
        ...
        if player1 > player2 {
            ...
            print("Round ends in a tie with \(player1.description).")
            print("\(player1) + \(player2)")
        }
